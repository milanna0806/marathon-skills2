...
export default function Home() {
  ...
  const [modal, setModal] = useState(null); // 'register' | 'bmi' | 'users' | 'edit'
  ...
  const handleSaveEdit = async () => {
    ...
    setEditLoading(true);
    const res = await fetch(`/api/participants/${editTarget.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      ...
      <header className="header">
        ...
        <div className="header-actions">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8 }}>
            ...
            {isAdmin && <span style={{ background: "#7c3aed", color: "#fff", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 10, letterSpacing: 1 }}>ADMIN</span>}
          ...
          <div className="modal-window modal-users">
            ...
            <div className="users-table-wrap">
              <table className="users-table">
                <thead>
                  <tr><th>ИМЯ</th><th>ФАМИЛИЯ</th><th>EMAIL</th><th>BMI</th><th>РОЛЬ</th>{isAdmin && <th>ДЕЙСТВИЯ</th>}</tr>
                </thead>
                <tbody>
                  {filtered.map(u=>(
                    <tr key={u.id}>
                      ...
                      {isAdmin && (
                        <td style={{whiteSpace:"nowrap"}}>
                          <button className="btn btn-ghost" style={{fontSize:12,padding:"3px 10px",marginRight:4}} onClick={()=>openEdit(u)}>✏️ Изменить</button>
                          <button className="btn" style={{fontSize:12,padding:"3px 10px",background:"#7f1d1d",color:"#fca5a5",border:"1px solid #991b1b",borderRadius:6}} onClick={()=>handleDelete(u.id, `${u.name} ${u.surname}`)}>🗑 Удалить</button>
                        </td>
                      )}
                    </tr>
                  ))}
      ...
      {modal === "edit" && isAdmin && editTarget && (
        <div className="modal-overlay open" onClick={(e) => e.target.className.includes("modal-overlay") && setModal("users")}>
          <div className="modal-window modal-register">
            <header className="modal-header">
              <div className="header-logo small">M</div>
              <span className="header-title">РЕДАКТИРОВАТЬ УЧАСТНИКА</span>
              <button className="btn btn-ghost" onClick={() => setModal("users")}>← Назад</button>
            </header>
            <div className="register-scroll">
              <div className="register-card">
                <div className="card-section-label">АДМИНИСТРАТОР</div>
                <h3 className="card-subhead">Редактирование: {editTarget.name} {editTarget.surname}</h3>
                <div className="card-divider"></div>
                <div className="form-columns">
                  <div className="form-col">
                    {[["👤 Имя:","name","text"],["👤 Фамилия:","surname","text"],["📧 Email:","email","email"]].map(([lbl,key,type])=>(
                      <div className="form-row" key={key}>
                        <label>{lbl}</label>
                        <div className="input-wrap">
                          <input type={type} className="input" value={editForm[key]} onChange={e=>setEditForm(f=>({...f,[key]:e.target.value}))} />
                        </div>
                      </div>
                    ))}
                    <div className="form-row">
                      <label>⚧ Пол:</label>
                      <div className="input-wrap">
                        <select className="input" value={editForm.gender} onChange={e=>setEditForm(f=>({...f,gender:e.target.value}))}>
                          <option>Мужской</option><option>Женский</option>
                        </select>
                      </div>
                    </div>
                ...
                <div className="form-btns">
                  <button className="btn btn-accent btn-lg" onClick={handleSaveEdit} disabled={editLoading}>
                    {editLoading ? "Сохранение..." : "💾 Сохранить изменения"}
                  </button>
                  <button className="btn btn-ghost btn-lg" onClick={() => setModal("users")}>✕ Отмена</button>
                </div>
              </div>
            </div>
          </div>
